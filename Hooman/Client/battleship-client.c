...
// ─── TCP ───────────────────────────────────────────────────────────────────
static struct tcp_pcb *conn_pcb = NULL;
static bool connected = false;

static void send_msg(const char *msg) {
    if (!conn_pcb || !connected) return;
    tcp_write(conn_pcb, msg, strlen(msg), TCP_WRITE_FLAG_COPY);
    tcp_output(conn_pcb);
}
// ───────────────────────────────────────────────────────────────────────────
...
void ResetCursor() {
    lights[MY_PLAYER][y][x].cursor = false;
    x = 0;
    y = 0;
}
...
static err_t on_connected(void *arg, struct tcp_pcb *pcb, err_t err) {
    if (err != ERR_OK) { printf("TCP connect failed: %d\n", err); return err; }
    connected = true;
...
int main() {
    ...
    err_t err = tcp_connect(conn_pcb, &server_addr, SERVER_PORT, on_connected);
