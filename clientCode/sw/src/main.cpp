...
static struct tcp_pcb *conn_pcb = nullptr;
...
static void SendMessage(const char *msg) {
    if (conn_pcb == nullptr || !connected) {
        printf("No TCP connection, dropping message: %s", msg);
        return;
    }

    err_t err = tcp_write(conn_pcb, msg, strlen(msg), TCP_WRITE_FLAG_COPY);
    if (err == ERR_OK) {
        tcp_output(conn_pcb);
    } else {
...
static err_t OnConnected(void *arg, struct tcp_pcb *pcb, err_t err) {
    ...
    printf("Connected to battleship server!\n");
    conn_pcb = pcb;
    connected = true;
...
static bool StartTcpClient() {
    ...
    conn_pcb = tcp_new_ip_type(IPADDR_TYPE_V4);
    if (conn_pcb == nullptr) {
        printf("tcp_new_ip_type failed\n");
        return false;
    }

    tcp_err(conn_pcb, OnTcpError);
    printf("Connecting to %s:%d...\n", SERVER_IP, SERVER_PORT);
    err_t err = tcp_connect(conn_pcb, &server_addr, SERVER_PORT, OnConnected);
    if (err != ERR_OK) {
        printf("tcp_connect start failed: %d\n", err);
        tcp_close(conn_pcb);
        conn_pcb = nullptr;
        return false;
    }
